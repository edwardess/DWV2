...
import ConfirmationModal from "@/components/ui/confirmation-modal";
  ...
  const RenderSecondColumn: React.FC<RenderSecondColumnProps> = ({
    ...
    const validateEmbedUrl = (url: string): boolean => {
      ...
      // Whitelist of allowed domains with path patterns
      const allowedSources = [
        { domain: "drive.google.com", pathPattern: /^\/file\/d\/.+\/preview$/ },
        { domain: "www.youtube.com", pathPattern: /^\/embed\/.+/ },
        { domain: "youtube.com", pathPattern: /^\/embed\/.+/ },
        { domain: "player.vimeo.com", pathPattern: /^\/video\/.+/ },
        { domain: "vimeo.com", pathPattern: /^\/\d+/ },
      ];
      ...
      try {
        ...
        const matchedSource = allowedSources.find(
          (source) => source.domain === parsedUrl.hostname
        );
  
        if (!matchedSource) return false;
  
        // Check if path matches expected pattern
        return matchedSource.pathPattern.test(parsedUrl.pathname);
      } catch {
        return false;
      }
    };
